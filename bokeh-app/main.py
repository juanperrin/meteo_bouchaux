...
for mois in liste_mois: # on boucle pour tous les mois du classeur
    ...
    annee_mois_jour = datetime.date(annee, mois, jour)
    ...
    hauteur_pluie_max = tab_clean['h_pluie'].max() # maximum de pluie journalière
    pluie_cumul = tab_clean['h_pluie'].sum() # calcul du cumul de pluie pour le mois
...
def make_plot_temp(source, title):
    plot2 = figure(plot_width=largeur_plot, plot_height=300, x_axis_type='datetime', x_range=(datetime.date(2019, 1, 1), datetime.date(2020, 1, 1)))
...
source_temp = ColumnDataSource(data={'x': data_all['dates'], 'y1': data_all['t_mini'], 'y2': data_all['t_maxi']})
